...
> Popcorn Hack
 ...
 Maybe when you are streaming so you get the stream in real time
...
<br>
 - 1.1.1.1.1 No five parts
 - 23.23.23.23 Yes five and within 0 to 255
 - 134.492.100.0 No outside 0 to 255
 - 255.256.55.255 No outside 0 to 255
 - 2.93.255.19 Yes has 4 parts and is from 0 to 255
...
- Unicast
- Broadcast
The answer is brodcast because brodcast communication is when you send a message to everyone on the network. Not a specific group of peopole or specified poeple, it's everyone on the network.
...
1. Three of the four following protocols are on the same layer. Identify which ones and what layer they are on, and why they are on each layer:
    - ASCII (see above for information) this one is not operating on a layer that we know of
    - FTP (facilitates transfer of files over the internet) application layer, layer 7 used to ransfer fils over a network
    - TLS (see HTTPS section) Transport layer layer 4, secure communication over a secure network
    - USB (permits data exchange between electronics) Its its own layer USB and it permits data exchange between electronics and data links
...
its located on application laye ron layer 7. and it provides netwrok services to end users on network
#### DNS
...
he should buy best.com and the sub domain is bob.is.the
#### HTTP and HTTPS
1. What is a difference between HTTP and HTTPS? HTTPS is a more secure version of HTTP
<br>
...
<br>
 ...
 - What are the benefits and disadvantages of this? its more secure but probably has more latency

#### TCP and UDP

1. Bob is setting up a video streaming service, and he needs the stream to be real time. 
<br>
 - What protocol should he use, TCP or UDP? Why? UDP because it has lower latency and fits his criteria
 - What are some cons of this protocol? Give an example of a potential issue. it has less error checkling which could give you data loss


2. TCP has error checking, which ensures that all packets arrive properly. Why is this important?
<br>
 - Give an example of how TCP ensures that there are no errors. if a file is lost than TCP makes suer it is retransmitted
...
<br>
 - How does Server B ensure that they have received any sent packets before Server A continues sending packets in TCP? In UDP? Server B aknowledges the sent and recieved data while UDP cant check which will cause data loss
 - What is another use of this? it can make sure you dont git overwhlmed with info by making sure the sent data has been recieved.